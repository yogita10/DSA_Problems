...
public class l001{
    ...
    public static int allPath(ArrayList<Edge>[] graph, int src, int dest, boolean[] vis, String psf){
        ...
        for(Edge e: graph[src]){
            if(!vis[e.v])
                count+=allPath(graph, e.v, dest, vis,psf+src);  // if psf+e.v to base case me add karne ki jarurat nahi h 
        }
    ...
    public static pair2 longestPath(ArrayList<Edge>[] graph, int src, int dest, boolean[] vis) {
        ...
        vis[src] = true;
        pair2 myAns = new pair2(-1, "");
        for (Edge e : graph[src]) {
            if (!vis[e.v]) {
                pair2 recAns = longestPath(graph, e.v, dest, vis);
                if (recAns.length != -1 && recAns.length + 1 > myAns.length) {
