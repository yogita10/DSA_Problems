...
public class l001Basic {
    ...
    public static int mazePath_HVD_multi(int sr, int sc, int er, int ec, ArrayList<String> ans, String psf) {
        ...
        int count = 0;
        for (int jump = 1; sr + jump <= er; jump++)
            count += mazePath_HVD_multi(sr + jump, sc + 0, er, ec, ans, psf + "V" + jump);
        for (int jump = 1; sr + jump <= er && sc + jump <= ec; jump++)
            count += mazePath_HVD_multi(sr + jump, sc + jump, er, ec, ans, psf + "D" + jump);
        for (int jump = 1; sc + jump <= ec; jump++)
            count += mazePath_HVD_multi(sr + 0, sc + jump, er, ec, ans, psf + "H" + jump);

        return count;
    }


    // Direction method - using vector theory 
    ...
    public static ArrayList<String> findPath(int[][] m, int n) {
        ...
        if(n==0 || m[0][0]==0 || m[n-1][n-1]==0) return res;
        int[][] dir={{1,0},{0,-1},{0,1},{-1,0}};
        String[] dirS={"D","L","R","U"};  // we want in lexicographical order  
        int count = floodFill(0,0,m,"",res,dir,dirS);
        return res;
    }


    // https://practice.geeksforgeeks.org/problems/special-matrix4201/1   - done in cpp
    // https://www.geeksforgeeks.org/rat-in-a-maze-with-multiple-steps-jump-allowed/?ref=rp  - done in cpp 
